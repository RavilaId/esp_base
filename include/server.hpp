...
// Manejo de los Archivos del Servidor Web Cloud.js
// -------------------------------------------------------------------
void handleDashboardJS(AsyncWebServerRequest *request) {
    if(!request->authenticate(device_old_user, device_old_password)) 
        return request->requestAuthentication(); 
    const char* dataType = "application/javascript"; 
    AsyncWebServerResponse *response = request->beginResponse_P(200, dataType,dashboard_js, dashboard_js_length);
    response->addHeader("Content-Encoding", "gzip");
    request->send(response);
}
...
void InitServer(){
 ...
 // -------------------------------------------------------------------
    ...
    // Método: GET 
    // -------------------------------------------------------------------
    server.on("/api/dash", HTTP_GET, [](AsyncWebServerRequest *request){
        if(!request->authenticate(device_old_user, device_old_password)) 
            return request->requestAuthentication();       
        const char* dataType = "application/json";
        String json = "";
    ...
    // Método: POST
    // -------------------------------------------------------------------
    server.on("/api/wifi", HTTP_POST, [](AsyncWebServerRequest * request){}, NULL, putRequestWiFi);
    // -------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------
        // -------------------------------------------------------------------
    // Cargar de Archivos complementarios ./js/dashboard.js
    // ------------------------------------------------------------------- 
    server.on("/js/dashboard.js",HTTP_GET,handleDashboardJS);
    // -------------------------------------------------------------------
